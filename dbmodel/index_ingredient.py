...
class Index_ingredient(dbbase):
    __table__ = Table('index_ingredient', md, autoload=True)
    ...
    def fetch_from_db(self, term):
        ...
        :return:
        """
        row = dbsession.query(Index_ingredient).filter_by(term=term).first()
        return row
